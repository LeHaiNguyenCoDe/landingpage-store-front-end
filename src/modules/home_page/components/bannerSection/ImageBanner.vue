    ...
    <div class="relative z-0 h-fit">
      ...
      <transition name="slide" mode="out-in">
        <div
          ...
          <span class="text-title f-text-10-20">{{ mainImage.price }}</span>
        </div>
      </transition>
    </div>

    <transition name="slide" mode="out-in">
      <img :src="rightImage.image" :key="rightImage.image" alt="" class="w-3vw h-40vw object-cover">
    </transition>
  </div>
</template>
